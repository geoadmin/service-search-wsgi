...
class BBox:
    ...
    def __eq__(self, other):
        if (
            self.minx == other.minx and self.miny == other.miny and self.maxx == other.maxx and
    ...
    def getIntersection(self, other):
        if (
            other.maxx < self.minx or other.minx > self.maxx or other.maxy < self.miny or
    ...
    def create_quads(self):
        ...
        quad1 = BBox(
            self.minx + newWidth, self.maxy - newHeight, self.minx + (2 * newWidth), self.maxy
        )
        quad2 = BBox(
            self.minx, self.maxy - (2 * newHeight), self.minx + newWidth, self.maxy - newHeight
        )
        quad3 = BBox(
            self.minx + newWidth,
            self.maxy - (2 * newHeight),
            self.minx + (2 * newWidth),
            self.maxy - newHeight
        )
